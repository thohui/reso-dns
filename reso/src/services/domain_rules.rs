...
impl DomainRulesService {
    ...
    pub fn match_rule(&self, name: &str) -> RuleMatch {
        ...
        if let Some(allow_id) = matchers.allow_list_matcher.exists(name).copied() {
            return RuleMatch::Allowed(allow_id);
        }

        match matchers.blocklist_matcher.exists(name).copied() {
            Some(blocked_id) => RuleMatch::Blocked(blocked_id),
            None => RuleMatch::NoMatch,
        }
    }
