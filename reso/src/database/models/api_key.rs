...
pub async fn find_by_hash(db: &CoreDatabasePool, hash: String) -> Result<Option<ApiKey>, DatabaseError> {
    db.interact(move |c| {
        c.query_row(
...
pub async fn insert(db: &CoreDatabasePool, api_key: ApiKey) -> Result<(), DatabaseError> {
    db.interact(move |c| {
            c.execute(
...
) -> Result<Page<(ApiKey, String)>, DatabaseError> {
    db.interact(move |c| {
...
pub async fn delete_by_id(db: &CoreDatabasePool, id: &EntityId<ApiKey>) -> Result<bool, DatabaseError> {
    let id = *id.id();
    let changed = db
...
mod tests {
    ...
    async fn test_delete() {
        ...
        assert!(
            list_with_username(&db.conn, 10, 0, None)
                .await
                .unwrap()
                .items
                .is_empty()
